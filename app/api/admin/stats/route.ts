...
export async function GET(request: NextRequest) {
  try {
    ...
    const [totalAssessments, completedAssessments, assessmentMessages] = await Promise.all([
    ...
    const [totalChatSessions, knowledgeSessions, chatMessages] = await Promise.all([
    ...
    const averageCompletion = totalAssessments > 0 
      ? (completedAssessments / totalAssessments) * 100 
      : 0

    const averageMessagesPerSession = totalChatSessions > 0
      ? chatMessages._count / totalChatSessions
      : 0

    const totalStorage = documents._sum.fileSize || 0
    const storageLimit = 1024 * 1024 * 1024 * 10 // 10GB limit

    const categoryStats = documentCategories.reduce((acc, cat) => {
