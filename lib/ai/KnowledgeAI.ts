  ...
  private async generateContextualResponse(
    ...
    const hasRelevantContext =
      ...
      searchResults[0].similarity >
        AI_PARAMETERS.KNOWLEDGE.HIGH_SIMILARITY_THRESHOLD;
    ...
    const contextChunks = searchResults
      .filter(
        (result) =>
          result.similarity > AI_PARAMETERS.KNOWLEDGE.MIN_SIMILARITY_FOR_CONTEXT
      )
...
${conversationContext}`;
    ...
    try {
      ...
      const responseContent =
        completion || SYSTEM_PROMPTS.FALLBACKS.KNOWLEDGE_ERROR;
      ...
      const confidence = Math.min(
        avgSimilarity * AI_PARAMETERS.KNOWLEDGE.CONFIDENCE_BOOST_FACTOR,
        AI_PARAMETERS.KNOWLEDGE.MAX_CONFIDENCE
      );
