...
export class SEOGenerator {
  ...
  generateMetadata(
    data: any,
    routeSEO?: Partial<SEOConfig>
  ): TemplateVariables["meta"] {
    ...
    const keywords = seoConfig.keywordsTemplate
      ? this.interpolate(seoConfig.keywordsTemplate, data)
          .split(",")
          .map((k) => k.trim())
      : undefined;
  ...
  private generateOpenGraph(
    ...
    return {
      title: ogConfig.titleTemplate
        ? this.interpolate(ogConfig.titleTemplate, data)
        : title,
      description: ogConfig.descriptionTemplate
        ? this.interpolate(ogConfig.descriptionTemplate, data)
        : description,
      image: ogConfig.imageTemplate
        ? this.interpolate(ogConfig.imageTemplate, data)
        : undefined,
  ...
  private generateStructuredData(data: any, config: StructuredDataConfig) {
    ...
    for (const [property, template] of Object.entries(config.properties)) {
      if (typeof template === "string") {
        ...
        structuredData[property] = template;
      }
    }
  ...
  generateMetaTags(metadata: TemplateVariables["meta"]): string {
    ...
    tags.push(
      `<meta property="og:title" content="${this.escapeHtml(metadata.openGraph.title)}" />`
    );
    tags.push(
      `<meta property="og:description" content="${this.escapeHtml(metadata.openGraph.description)}" />`
    );
    tags.push(
    ...
    tags.push(
      `<meta name="twitter:title" content="${this.escapeHtml(metadata.openGraph.title)}" />`
    );
    tags.push(
      `<meta name="twitter:description" content="${this.escapeHtml(metadata.openGraph.description)}" />`
    );
